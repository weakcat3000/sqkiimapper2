...
import { initJigsawFeature, openJigsawWorkspace } from './features/jigsaw/jigsawFeature.js';
      ...
      const POWER_SAVE_BOOT_ENABLED = (() => {
        try { return localStorage.getItem(POWER_SAVE_LS_KEY) === '1'; } catch { return false; }
      })();
      ...
      let powerSavingEnabled = POWER_SAVE_BOOT_ENABLED;
        ...
        ensureCatLottieLoaded = () => {
          ...
          try {
            catAnimation = window.lottie.loadAnimation({
              ...
              renderer: 'canvas',
              loop: true,
              autoplay: !powerSavingEnabled && !document.hidden,
              path: catLottieSrc,
              rendererSettings: {
                clearCanvas: true,
...
`;
      ...
      function trashSvg() {
        return `<svg viewBox="0 0 24 24"><path d="M8 5h8"/><path d="M10 3h4"/><path d="M6 5l1 14a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2l1-14"/><path d="M10 10v6"/><path d="M14 10v6"/></svg>`;
      }
          ...
          if (visibleOnMap) {
            ...
            for (const it of (entry.items || [])) {
              ...
              trashBtn.addEventListener('click', () => {
                deleteFeatureByFid(entry, it.fid, { confirmDelete: true, closeEditorOnDone: true });
              });
              ...
              row.append(ico, name, moveBtn, trashBtn, eyeBtn);
      ...
      function openStyleEditor(entry, fid, screenX, screenY) {
        ...
        if (isCircle) {
          ...
          sePredictExact = document.createElement('button');
          sePredictExact.className = 'btn small';
          sePredictExact.style.cssText = 'width:100%;margin-top:4px;background:rgba(168,85,247,0.14);border-color:#c084fc;color:#f3e8ff;';
          sePredictExact.textContent = '✨ Predict Exact';

          sePredictExact.onclick = () => {
            let center = p._circleCenter;
            let radius = p._circleRadius;
    ...
    `;
      ...
      function roomPresencePayload() {
        ...
        if (roomPresenceLocationAvailable) {
          payload.share_location = roomPresenceShouldShareLocation();
      ...
      async function refreshRoomPresence() {
        ...
        if (!roomPresenceAvailable) {
          setConnectedUsersList([{
            ...
            country: userInfo.countryCode,
            ip: userInfo.ip,
            share_location: roomPresenceShouldShareLocation(),
      ...
      function focusRoomPresenceUser(user) {
        const lng = Number(user?.lng);
        const lat = Number(user?.lat);
        ...
        if (engine === 'gl' && mapgl?.flyTo) {
          mapgl.flyTo({
            center: [lng, lat],
            zoom: Math.max(Number(mapgl.getZoom?.() || 0), 17),
            speed: 1.15,
            essential: true
          });
          return true;
        }
        if (engine === 'leaf' && mapleaf?.flyTo) {
          mapleaf.flyTo([lat, lng], Math.max(Number(mapleaf.getZoom?.() || 0), 17), { duration: 0.9 });
          return true;
        }
        return false;
      }

      async function refreshAndFocusRoomPresenceUser(sessionId) {
        if (!sessionId || !currentRoomCode) return;
        const escapedSessionId = typeof CSS !== 'undefined' && typeof CSS.escape === 'function'
          ? CSS.escape(String(sessionId))
          : String(sessionId).replace(/"/g, '\\"');
        const chip = document.querySelector(`.connection-player-chip[data-presence-session="${escapedSessionId}"]`);
        if (chip) chip.classList.add('loading');
        try {
          await refreshRoomPresence();
          const user = (window.connectedUsersList || []).find((entry) => String(entry?.session_id || '') === String(sessionId));
          if (!user || !user.share_location || !Number.isFinite(Number(user.lat)) || !Number.isFinite(Number(user.lng))) {
            alert('That player is not sharing a live location right now.');
            return;
          }
          focusRoomPresenceUser(user);
        } catch (error) {
          console.warn('Failed to refresh player location:', error);
          alert('Could not refresh that player location right now.');
        } finally {
      ...
      document.getElementById('connection-players')?.addEventListener('click', async (e) => {
        const chip = e.target?.closest?.('.connection-player-chip[data-presence-session]');
        if (!chip) return;
        e.preventDefault();
        e.stopPropagation();
        if (chip.disabled) return;
        await refreshAndFocusRoomPresenceUser(chip.dataset.presenceSession);
      });
      ...
      const jigsawFinderCameraInput = document.getElementById('jigsaw-finder-camera-input');
      ...
      function getJigsawFinderPreviewFilter() {
      ...
      function applyJigsawFinderCanvasFilter() {
        ...
        jigsawFinderCanvas.style.webkitFilter = filter;
      }
      function clearJigsawFinderCanvasFilter() {
        if (!jigsawFinderCanvas) return;
      ...
      function downloadJigsawFinderCanvas() {
        ...
        const { css } = getJigsawFinderPreviewFilter();
        const shouldBakeFilter = (jigsawFinderCanvas.style.filter || 'none') !== 'none';
        let href = jigsawFinderCanvas.toDataURL('image/png');
        if (shouldBakeFilter) {
          const exportCanvas = document.createElement('canvas');
          exportCanvas.width = jigsawFinderCanvas.width;
          exportCanvas.height = jigsawFinderCanvas.height;
          const exportCtx = exportCanvas.getContext('2d', { alpha: false });
          if (exportCtx) {
            exportCtx.filter = css;
            exportCtx.drawImage(jigsawFinderCanvas, 0, 0);
      ...
      function openJigsawFinderCameraPicker() {
        if (!jigsawFinderCameraInput) return;
        jigsawFinderCameraInput.value = '';
        jigsawFinderCameraInput.click();
      }
      function openJigsawFinderGalleryPicker() {
        if (!jigsawFinderInput) return;
        jigsawFinderInput.value = '';
        jigsawFinderInput.click();
      }
      function handleJigsawFinderPickerChange(event) {
      ...
      jigsawFinderCameraBtn?.addEventListener('click', openJigsawFinderCameraPicker);
...
<body>
  ...
  <script>
    (function () {
      ...
      /* ================= Shrink Predictor ================= */
      const shrinkPredictorModal = byId('shrink-predictor-modal');
      const shrinkPredictorCloseBtn = byId('shrink-predictor-close');
      ...
      function shrinkPredictorFormatDistance(value) {
      ...
      function shrinkPredictorSignedOffsetMeters(origin, target) {
        const ox = Number(origin?.lng);
        const oy = Number(origin?.lat);
        const tx = Number(target?.lng);
        const ty = Number(target?.lat);
        if (![ox, oy, tx, ty].every(Number.isFinite)) return { east: 0, north: 0 };

        const east = turf.distance([ox, oy], [tx, oy], { units: 'kilometers' }) * 1000 * (tx >= ox ? 1 : -1);
        const north = turf.distance([ox, oy], [ox, ty], { units: 'kilometers' }) * 1000 * (ty >= oy ? 1 : -1);
        return { east, north };
      }

      function shrinkPredictorDestination(origin, east, north) {
        const lng = Number(origin?.lng);
        const lat = Number(origin?.lat);
        if (![lng, lat, east, north].every(Number.isFinite)) return null;
        const distanceMeters = Math.hypot(east, north);
        if (!Number.isFinite(distanceMeters) || distanceMeters <= 0.001) return { lat, lng };
        const bearing = Math.atan2(east, north) * 180 / Math.PI;
        const destination = turf.destination([lng, lat], distanceMeters / 1000, bearing, { units: 'kilometers' });
        return {
          lat: Number(destination?.geometry?.coordinates?.[1]),
          lng: Number(destination?.geometry?.coordinates?.[0])
        };
      }

      function shrinkPredictorMetersBetween(a, b) {
      ...
      function shrinkPredictorWeightedPercentile(values, weights, percentile = 0.7) {
        const items = values
          .map((value, index) => ({ value: Number(value), weight: Math.max(0, Number(weights?.[index]) || 0) }))
          .filter((item) => Number.isFinite(item.value) && item.weight > 0)
          .sort((a, b) => a.value - b.value);
        if (!items.length) return NaN;
        const totalWeight = items.reduce((sum, item) => sum + item.weight, 0);
        const target = totalWeight * shrinkPredictorClamp(percentile, 0, 1);
        let running = 0;
      ...
      function shrinkPredictorWeightedMean(values = [], weights = []) {
      ...
      function shrinkPredictorComputeHeatmapPeak(neighbors = [], meanOffset = [0, 0], radiusMeters = 0, uncertaintyMeters = 0) {
        const usableNeighbors = (Array.isArray(neighbors) ? neighbors : [])
          .filter((neighbor) => Array.isArray(neighbor?.output) && neighbor.output.length >= 2)
          .map((neighbor) => ({
            east: Number(neighbor.output[0]) || 0,
            north: Number(neighbor.output[1]) || 0,
            weight: Math.max(0.0001, Number(neighbor.weight) || 0.0001)
          }));

        if (!usableNeighbors.length) {
          return {
            peakOffset: [Number(meanOffset?.[0]) || 0, Number(meanOffset?.[1]) || 0],
            heatRadiusMeters: Math.max(20, Number(radiusMeters) * 0.22 || 20)
          };
        }

        const anchorRadius = Math.max(30, Number(radiusMeters) || 30);
        const baseSigmaMeters = Math.max(20, anchorRadius * 0.22);
        const uncertaintyScale = 1 + (Math.max(0, Number(uncertaintyMeters) || 0) / 100) * 1.5;
        const sigmas = usableNeighbors.map((neighbor) => baseSigmaMeters * uncertaintyScale / Math.max(0.3, neighbor.weight));

        const extentMeters = Math.max(
          anchorRadius,
          ...usableNeighbors.map((neighbor, index) => Math.hypot(neighbor.east, neighbor.north) + sigmas[index] * 2.2)
        );
        const gridSize = 41;
        const stepMeters = (extentMeters * 2) / (gridSize - 1);
        let bestScore = -Infinity;
        let bestOffset = [Number(meanOffset?.[0]) || 0, Number(meanOffset?.[1]) || 0];

        for (let row = 0; row < gridSize; row += 1) {
          const north = -extentMeters + row * stepMeters;
          for (let col = 0; col < gridSize; col += 1) {
            const east = -extentMeters + col * stepMeters;
            let score = 0;
            for (let index = 0; index < usableNeighbors.length; index += 1) {
              const neighbor = usableNeighbors[index];
              const sigma = Math.max(6, sigmas[index]);
              const dx = east - neighbor.east;
              const dy = north - neighbor.north;
              score += neighbor.weight * Math.exp(-((dx * dx) + (dy * dy)) / (2 * sigma * sigma));
            }
            if (score > bestScore) {
              bestScore = score;
              bestOffset = [east, north];
            }
          }
        }

        return {
          peakOffset: bestOffset,
          heatRadiusMeters: baseSigmaMeters * uncertaintyScale
        };
      }
      ...
      function shrinkPredictorBlendNeighborCloud(models = []) {
        ...
        for (const modelItem of (Array.isArray(models) ? models : [])) {
          const modelWeight = Math.max(0.0001, Number(modelItem?.weight) || 0.0001);
          const neighbors = Array.isArray(modelItem?.neighbors) ? modelItem.neighbors : [];
          for (const neighbor of neighbors) {
            if (!neighbor?.sampleId) continue;
            const nextWeight = modelWeight * Math.max(0.0001, Number(neighbor.weight) || 0.0001);
            if (merged.has(neighbor.sampleId)) {
              const current = merged.get(neighbor.sampleId);
              current.weight += nextWeight;
              current.distance = Math.min(current.distance, Number(neighbor.adjustedDistance ?? neighbor.distance) || current.distance || 0);
            } else {
              merged.set(neighbor.sampleId, {
                ...neighbor,
                weight: nextWeight,
                distance: Number(neighbor.adjustedDistance ?? neighbor.distance) || 0
              });
            }
          }
        }
      ...
      function shrinkPredictorNormalizeSteps(rawSteps = []) {
        return (Array.isArray(rawSteps) ? rawSteps : [])
          .map((step, index) => {
            ...
            const lng = Number(step?.lng);
            const radiusMeters = Number(step?.radiusMeters);
            if (!Number.isFinite(lat) || !Number.isFinite(lng) || !Number.isFinite(radiusMeters) || radiusMeters <= 0) return null;
            const stepNo = Number(step?.step);
            return {
              ...step,
              step: Number.isFinite(stepNo) ? stepNo : index + 1,
              lat,
              lng,
              radiusMeters
            };
          ...
          .filter(Boolean)
          .sort((a, b) => {
            const stepDiff = (Number(a.step) || 0) - (Number(b.step) || 0);
      ...
      function shrinkPredictorBuildSequenceProfile(steps = []) {
        const normalizedSteps = shrinkPredictorNormalizeSteps(steps);
        ...
        const anchor = normalizedSteps[normalizedSteps.length - 1];
        const startRadiusMeters = Math.max(1, Number(origin.radiusMeters) || 1);
        const anchorRadiusMeters = Math.max(1, Number(anchor.radiusMeters) || 1);
        const driftMeters = shrinkPredictorMetersBetween(origin, anchor) || 0;
      ...
      function shrinkPredictorProfileDistance(queryProfile = {}, sampleProfile = {}) {
        const metrics = shrinkPredictorCompareProfiles(queryProfile, sampleProfile);
        return (
          (metrics.radiusDelta * 3.2)
          + (metrics.driftDelta * 2.2)
          + (metrics.cadenceDelta * 1.25)
          + (metrics.elapsedRatioLog * 1.75)
          + (metrics.shrinkRateRatioLog * 1.55)
        );
      }

      function shrinkPredictorBuildRecordMetrics(steps = [], exactCoords = null) {
        const normalizedSteps = shrinkPredictorNormalizeSteps(steps);
        if (!normalizedSteps.length) return null;
        const origin = normalizedSteps[0];
        const finalStep = normalizedSteps[normalizedSteps.length - 1];
        const startRadiusMeters = Math.max(1, Number(origin.radiusMeters) || 1);
        const floorRadiusMeters = Math.max(1, Number(finalStep.radiusMeters) || 1);
        const shrinkableRadiusMeters = Math.max(1, startRadiusMeters - floorRadiusMeters);
        const startTs = coinDbStepTimestamp(origin);
        const endTs = coinDbStepTimestamp(finalStep);
        const totalDurationSeconds = Number.isFinite(startTs) && Number.isFinite(endTs) && startTs < Number.MAX_SAFE_INTEGER && endTs < Number.MAX_SAFE_INTEGER
          ? Math.max(0, (endTs - startTs) / 1000)
          : Math.max(0, normalizedSteps.length - 1);
        const totalDriftMeters = exactCoords
          ? (shrinkPredictorMetersBetween(origin, { lat: Number(exactCoords.lat), lng: Number(exactCoords.lng) }) || 0)
          : 0;
        return {
          startRadiusMeters,
          floorRadiusMeters,
          floorRadiusRatio: floorRadiusMeters / startRadiusMeters,
          shrinkableRadiusMeters,
          totalDurationSeconds,
          totalDriftMeters
        };
      }

      function shrinkPredictorEstimateFloorState(observedSteps = [], neighbors = []) {
        const normalizedSteps = shrinkPredictorNormalizeSteps(observedSteps);
        const origin = normalizedSteps[0];
        const anchor = normalizedSteps[normalizedSteps.length - 1];
        ...
        const weights = [];
        const floorRatios = [];
        const remainingProgresses = [];
        const totalDurationRatios = [];

        for (const neighbor of (Array.isArray(neighbors) ? neighbors : [])) {
          const weight = Math.max(0.0001, Number(neighbor?.weight) || 0.0001);
          const metrics = neighbor?.recordMetrics || {};
          const floorRatio = Number(metrics.floorRadiusRatio);
          if (Number.isFinite(floorRatio) && floorRatio > 0 && floorRatio < 1) {
            weights.push(weight);
            floorRatios.push(floorRatio);
          }
          const remainingProgress = Number(neighbor?.remainingProgress);
          if (Number.isFinite(remainingProgress)) remainingProgresses.push({ value: remainingProgress, weight });
          const totalDurationRatio = Number(neighbor?.totalDurationRatio);
          if (Number.isFinite(totalDurationRatio)) totalDurationRatios.push({ value: totalDurationRatio, weight });
        }

        const estimatedFloorRatioRaw = shrinkPredictorWeightedMean(floorRatios, weights);
        const estimatedFloorRatio = shrinkPredictorClamp(
          Number.isFinite(estimatedFloorRatioRaw) ? estimatedFloorRatioRaw : 0.38,
          0.08,
          Math.max(0.09, (anchorRadiusMeters / startRadiusMeters) - 0.005)
        );
        const estimatedFloorRadiusMeters = Math.min(
          anchorRadiusMeters * 0.995,
          Math.max(12, startRadiusMeters * estimatedFloorRatio)
        );
        const shrinkableRadiusMeters = Math.max(1, startRadiusMeters - estimatedFloorRadiusMeters);
        const observedShrinkMeters = Math.max(0, startRadiusMeters - anchorRadiusMeters);
        const progressToFloor = shrinkPredictorClamp(observedShrinkMeters / shrinkableRadiusMeters, 0, 1);
        const remainingProgress = Math.max(0, 1 - progressToFloor);
        const sampleRemainingProgressMean = remainingProgresses.length
          ? shrinkPredictorWeightedMean(remainingProgresses.map((item) => item.value), remainingProgresses.map((item) => item.weight))
          : remainingProgress;
        const totalDurationRatioMean = totalDurationRatios.length
          ? shrinkPredictorWeightedMean(totalDurationRatios.map((item) => item.value), totalDurationRatios.map((item) => item.weight))
          : 1;

        return {
          startRadiusMeters,
          anchorRadiusMeters,
          estimatedFloorRadiusMeters,
          estimatedFloorRatio,
          progressToFloor,
          remainingProgress,
          sampleRemainingProgressMean: Math.max(0.02, Number(sampleRemainingProgressMean) || remainingProgress || 0.02),
          totalDurationRatioMean: Math.max(0.1, Number(totalDurationRatioMean) || 1)
        };
      }

      function shrinkPredictorBuildCandidateSpots(anchor, centerPoint, neighbors = [], radiusMeters = 0) {
        if (!anchor || !centerPoint) return [];
        const safeRadiusMeters = Math.max(0, Number(radiusMeters) || 0);
        const pointMap = new Map();
        const sources = [
          { point: centerPoint, weight: 1.35, source: 'heatmap' },
          ...(Array.isArray(neighbors) ? neighbors.slice(0, 12).map((neighbor) => ({
            point: neighbor.projectedPoint,
            weight: Math.max(0.0001, Number(neighbor.weight) || 0.0001),
            source: 'memory'
          })) : [])
        ];
        ...
        for (const source of sources) {
          ...
          const distanceFromAnchor = shrinkPredictorMetersBetween(anchor, point) || 0;
          if (safeRadiusMeters > 0 && distanceFromAnchor > safeRadiusMeters * 1.001) continue;
          const key = `${Number(point.lat).toFixed(6)}:${Number(point.lng).toFixed(6)}`;
          const current = pointMap.get(key);
          if (current) {
            current.seedWeight += source.weight;
          } else {
            pointMap.set(key, {
              point: { lat: Number(point.lat), lng: Number(point.lng) },
              seedWeight: source.weight,
      ...
      function shrinkPredictorFindCurrentObservation(fid, center, radiusMeters, featureProps = {}) {
        ...
        const coinId = coinDbRootCoinId(featureProps?._coinId || featureProps?.coin_id || featureProps?.fid || featureProps?._gid || '');
        const coinLabel = coinDbCanonicalLabel(featureProps?._coinLabel || featureProps?.coin_label || featureProps?.name || featureProps?.title || '');
        let matchedGroup = groups.find((group) => group.steps.some((step) => String(step?.fid || '') === String(fid)));

        if (!matchedGroup && coinId) {
          matchedGroup = groups.find((group) => coinDbRootCoinId(group.coinId) === coinId);
        }

        if (!matchedGroup && coinLabel) {
          matchedGroup = groups.find((group) => coinDbCanonicalLabel(group.coinLabel) === coinLabel);
        }

        let observedSteps = shrinkPredictorNormalizeSteps(matchedGroup?.steps || []);
        const matchedIndex = observedSteps.findIndex((step) => String(step?.fid || '') === String(fid));
        if (matchedIndex >= 0) {
          observedSteps = observedSteps.slice(0, matchedIndex + 1);
        }

        if (!observedSteps.length) {
          observedSteps = [{
            step: 1,
            lat: Number(center?.[1]),
            lng: Number(center?.[0]),
            radiusMeters: Number(radiusMeters),
            featureName: String(featureProps?.name || 'Selected circle')
          }].filter((step) => Number.isFinite(step.lat) && Number.isFinite(step.lng) && Number.isFinite(step.radiusMeters) && step.radiusMeters > 0);
        }

        const lastStep = observedSteps[observedSteps.length - 1] || null;
        return {
          fid: String(fid || ''),
          coinId,
        ...
        shrinkPredictorMlModelPromise = (async () => {
          ...
          const entries = (data || [])
            .map((entry) => {
              ...
              const recordMetrics = shrinkPredictorBuildRecordMetrics(steps, exactCoords);
              return { entry, steps, exactCoords, recordMetrics };
            })
            .filter(Boolean);
          ...
          for (const record of entries) {
            for (let end = SHRINK_PREDICTOR_MIN_PREFIX_STEPS; end <= record.steps.length; end += 1) {
              ...
              const prefixProfile = shrinkPredictorBuildSequenceProfile(prefix);
              const startRadiusMeters = Math.max(1, Number(record.recordMetrics?.startRadiusMeters) || Number(prefix[0]?.radiusMeters) || 1);
              const floorRadiusMeters = Math.max(1, Number(record.recordMetrics?.floorRadiusMeters) || Number(record.steps[record.steps.length - 1]?.radiusMeters) || 1);
              const shrinkableRadiusMeters = Math.max(1, startRadiusMeters - floorRadiusMeters);
              const progressToFloor = shrinkPredictorClamp(
                (startRadiusMeters - Math.max(1, Number(last?.radiusMeters) || 1)) / shrinkableRadiusMeters,
                0,
                1
              );
              const remainingProgress = Math.max(0, 1 - progressToFloor);
              const startTs = coinDbStepTimestamp(record.steps[0]);
              const endTs = coinDbStepTimestamp(record.steps[record.steps.length - 1]);
              const lastTs = coinDbStepTimestamp(last);
              const totalDurationSeconds = Number(record.recordMetrics?.totalDurationSeconds) || 0;
              const elapsedSeconds = Number.isFinite(startTs) && Number.isFinite(lastTs) && startTs < Number.MAX_SAFE_INTEGER && lastTs < Number.MAX_SAFE_INTEGER
                ? Math.max(0, (lastTs - startTs) / 1000)
                : Math.max(0, end - 1);
              const remainingDurationSeconds = Number.isFinite(endTs) && Number.isFinite(lastTs) && endTs < Number.MAX_SAFE_INTEGER && lastTs < Number.MAX_SAFE_INTEGER
                ? Math.max(0, (endTs - lastTs) / 1000)
                : Math.max(0, totalDurationSeconds - elapsedSeconds);
              samples.push({
                sampleId: `${record.entry.id}:${end}`,
                entry: record.entry,
                roomCode: String(record.entry?.room_code || '').trim().toLowerCase(),
                prefix,
                ...
                progressToFloor,
                remainingProgress,
                totalDurationRatio: totalDurationSeconds > 0 ? (elapsedSeconds / totalDurationSeconds) : progressToFloor,
      ...
      function shrinkPredictorPredictNeighbors(model, queryStd, indices = null, limit = SHRINK_PREDICTOR_K, options = {}) {
        ...
        const queryRoomCode = String(options?.roomCode || '').trim().toLowerCase();
        const excludeEntryId = String(options?.excludeEntryId || '');
        const penaltyWeights = options?.penaltyWeights || {};
        const radiusPenaltyWeight = Number.isFinite(Number(penaltyWeights.radius)) ? Number(penaltyWeights.radius) : 3.2;
        ...
        const sameRoomSamples = queryRoomCode
        ...
        const neighbors = pool
          ...
          .map((sample) => ({
            ...sample,
            weight: 1 / Math.max(1e-6, sample.adjustedDistance + 0.015)
          }));
        ...
        const predictionScaled = neighbors.reduce((acc, neighbor) => {
          acc[0] += (Number(neighbor.outputNorm?.[0]) || 0) * neighbor.weight;
          acc[1] += (Number(neighbor.outputNorm?.[1]) || 0) * neighbor.weight;
          return acc;
        }, [0, 0]).map((value) => (value / totalWeight) * queryAnchorRadiusMeters);
      ...
      function shrinkPredictorSolveCore(observation, model, options = {}) {
        const observedSteps = observation?.observedSteps || [];
        const anchor = observedSteps[observedSteps.length - 1];
        if (!anchor || !Array.isArray(model?.samples) || !model.samples.length) return null;

        const config = options?.config || shrinkPredictorGetDefaultConfig();
        ...
        const neighborOptions = {
          ...
          roomFactor: config.roomFactor
        };
        const fullModel = shrinkPredictorPredictNeighbors(model, queryStd, model.indices.full, SHRINK_PREDICTOR_K, neighborOptions);
        const shapeModel = shrinkPredictorPredictNeighbors(model, queryStd, model.indices.shape, Math.max(6, Math.ceil(SHRINK_PREDICTOR_K * 0.8)), neighborOptions);
        const motionModel = shrinkPredictorPredictNeighbors(model, queryStd, model.indices.motion, Math.max(6, Math.ceil(SHRINK_PREDICTOR_K * 0.8)), neighborOptions);
        const ensembleWeights = config.ensemble || {};
        const ensemblePredictions = [
          { weight: Number(ensembleWeights.fullRaw) || 0, offset: fullModel.prediction },
          { weight: Number(ensembleWeights.fullScaled) || 0, offset: fullModel.predictionScaled },
          { weight: Number(ensembleWeights.shapeRaw) || 0, offset: shapeModel.prediction },
          { weight: Number(ensembleWeights.shapeScaled) || 0, offset: shapeModel.predictionScaled },
          { weight: Number(ensembleWeights.motionRaw) || 0, offset: motionModel.prediction },
          { weight: Number(ensembleWeights.motionScaled) || 0, offset: motionModel.predictionScaled }
        ...
        const blendWeights = config.blend || {};
        const blendedNeighbors = shrinkPredictorBlendNeighborCloud([
          { weight: Number(blendWeights.full) || 0.52, neighbors: fullModel.neighbors },
          { weight: Number(blendWeights.shape) || 0.28, neighbors: shapeModel.neighbors },
          { weight: Number(blendWeights.motion) || 0.2, neighbors: motionModel.neighbors }
        ]);
        const floorState = shrinkPredictorEstimateFloorState(observedSteps, blendedNeighbors);
        const progressScale = floorState
          ? shrinkPredictorClamp(
            (Number(floorState.remainingProgress) || 0.02) / Math.max(0.02, Number(floorState.sampleRemainingProgressMean) || 0.02),
            0.65,
            1.45
          )
          : 1;
        const scaledMeanOffset = [
          meanOffset[0] * progressScale,
        ...
        const uncertaintyMeters = Math.max(12, (modelStd * progressScale) + fullModel.spread);
        const agreement = 1 / (1 + modelStd / 80);
        const heatmapPeak = shrinkPredictorComputeHeatmapPeak(scaledNeighbors, scaledMeanOffset, anchor.radiusMeters, uncertaintyMeters);
        const boundedPeakOffset = shrinkPredictorClampOffsetToCircle(heatmapPeak.peakOffset, anchor.radiusMeters);
        const rawPredictedPoint = shrinkPredictorDestination(anchor, boundedPeakOffset[0], boundedPeakOffset[1]) || { lat: anchor.lat, lng: anchor.lng };
        const candidateSpots = shrinkPredictorBuildCandidateSpots(anchor, rawPredictedPoint, scaledNeighbors, anchor.radiusMeters);
        const predictedPoint = candidateSpots[0]?.point || rawPredictedPoint;
        const predictedOffsetMeters = shrinkPredictorSignedOffsetMeters(anchor, predictedPoint);
        ...
        return {
          ...
          shapeModel,
          motionModel,
          blendedNeighbors: scaledNeighbors,
          agreement,
          uncertaintyMeters,
          heatmapPeak,
          predictedPoint,
          predictedOffsetMeters: [predictedOffsetMeters.east, predictedOffsetMeters.north],
          progressScale,
          floorState,
          candidateSpots,
      ...
      function shrinkPredictorBacktestConfig(model, calibrationSelection, config, observation = null, options = {}) {
        ...
        for (const sample of samples) {
          ...
          byEntry.get(entryId).push(sample);
        }

        for (const [entryId, entrySamples] of byEntry.entries()) {
          const weightedErrors = [];
          const weightedStateErrors = [];
          for (const sample of entrySamples) {
            const sampleObservation = {
              observedSteps: sample.prefix,
              matchedGroup: { roomCode: sample.roomCode || '' }
            };
            const solved = shrinkPredictorSolveCore(sampleObservation, model, {
              config,
              roomCode: sample.roomCode || '',
              excludeEntryId: entryId
            });
            if (!solved?.predictedOffsetMeters) continue;
            const errorMeters = Math.hypot(
              (Number(solved.predictedOffsetMeters[0]) || 0) - (Number(sample.output?.[0]) || 0),
              (Number(solved.predictedOffsetMeters[1]) || 0) - (Number(sample.output?.[1]) || 0)
            );
            if (!Number.isFinite(errorMeters)) continue;
            const stateDistance = shrinkPredictorProfileDistance(targetProfile, sample.profile || {});
            const stateWeight = 1 / Math.max(0.2, 0.5 + (stateDistance * 2.4));
            weightedErrors.push({ value: errorMeters, weight: stateWeight });
            ...
            if (shrinkPredictorIsStateAligned(targetProfile, sample.profile || {}, 'near')) {
              weightedStateErrors.push({ value: errorMeters, weight: stateWeight * 1.2 });
            }
          }
          if (!weightedErrors.length) continue;
          const coinWeightedMean = shrinkPredictorWeightedMean(
            weightedErrors.map((item) => item.value),
            weightedErrors.map((item) => item.weight)
          );
          const coinStateMean = weightedStateErrors.length
            ? shrinkPredictorWeightedMean(
              weightedStateErrors.map((item) => item.value),
              weightedStateErrors.map((item) => item.weight)
            )
            : coinWeightedMean;
          coinErrors.push({
            entryId,
        ...
        const perCoinMeans = coinErrors.map((item) => item.meanErrorMeters);
        const perCoinStateMeans = coinErrors.map((item) => item.stateMeanErrorMeters);
        const weights = perCoinMeans.map(() => 1);
        const meanErrorMeters = shrinkPredictorMean(perCoinMeans) || Infinity;
        const medianErrorMeters = shrinkPredictorWeightedPercentile(perCoinMeans, weights, 0.5) || Infinity;
        const p70ErrorMeters = shrinkPredictorWeightedPercentile(perCoinMeans, weights, 0.7) || Infinity;
        const exactStateMeanErrorMeters = exactStateErrors.length ? (shrinkPredictorMean(exactStateErrors) || Infinity) : Infinity;
        const nearStateMeanErrorMeters = perCoinStateMeans.length ? (shrinkPredictorMean(perCoinStateMeans) || Infinity) : (nearStateErrors.length ? (shrinkPredictorMean(nearStateErrors) || Infinity) : Infinity);
        return {
      ...
      function shrinkPredictorBuildResult(observation, model, calibration = null) {
        const observedSteps = observation?.observedSteps || [];
        const anchor = observedSteps[observedSteps.length - 1];
        ...
        const solved = shrinkPredictorSolveCore(observation, model, {
          config: calibration?.config || shrinkPredictorGetDefaultConfig()
        });
        if (!solved) return null;

        const matches = solved.blendedNeighbors.slice(0, 8).map((neighbor) => {
          const projectedOffset = Array.isArray(neighbor.scaledOutput) ? neighbor.scaledOutput : neighbor.output;
          const projectedPoint = shrinkPredictorDestination(anchor, projectedOffset[0], projectedOffset[1]) || solved.predictedPoint;
          return {
            history: {
            ...
            projectedOffset: {
              east: projectedOffset[0],
              north: projectedOffset[1]
            },
            remainingDistanceMeters: Math.hypot(projectedOffset[0], projectedOffset[1]),
        ...
        return {
          ...
          candidateSpots: solved.candidateSpots || [],
          estimatedFloorRadiusMeters: Number(solved.floorState?.estimatedFloorRadiusMeters) || 0,
          progressToFloor: Number(solved.floorState?.progressToFloor) || 0,
          matches,
      ...
      function shrinkPredictorRenderSummary(observation, historyCount = 0) {
        if (!shrinkPredictorSummaryEl) return;
        if (!observation?.observedSteps?.length) {
          shrinkPredictorSummaryEl.innerHTML = '';
          return;
        }

        const lastStep = observation.observedSteps[observation.observedSteps.length - 1];
        const chips = [
      ...
      function shrinkPredictorRenderResults(result) {
        ...
        const confidenceLabel = shrinkPredictorConfidenceLabel(result);
        const predictedCoordText = `${coinDbFormatCoord(result.predictedPoint.lat)}, ${coinDbFormatCoord(result.predictedPoint.lng)}`;
        ...
        const progressPct = Math.round(shrinkPredictorClamp(Number(result.progressToFloor) || 0, 0, 1) * 100);
        const summaryLines = [
        ...
        const candidateHtml = result.candidateSpots?.length
          ? `
            <section class="shrink-predictor-matches">
              <h4>Top Candidate Spots</h4>
              <div class="shrink-predictor-match-list">
                ${result.candidateSpots.map((spot) => `
                  <div class="shrink-predictor-match">
                    <div class="shrink-predictor-match-top">
                      <div class="shrink-predictor-match-title">${spot.rank}. Candidate Spot</div>
                      <span class="shrink-predictor-match-score">${Math.round(Math.max(1, Math.min(99, spot.score * 4)))}% fit</span>
                    </div>
                    <div class="shrink-predictor-match-meta">
                      <span>${escapeHtml(`${coinDbFormatCoord(spot.point.lat)}, ${coinDbFormatCoord(spot.point.lng)}`)}</span>
                      <span>${spot.source === 'heatmap' ? 'Heatmap peak' : 'Historical cluster'}</span>
                    </div>
                  </div>
                `).join('')}
              </div>
        ...
        const matchHtml = result.matches.length
          ? result.matches.map((match, index) => {
            const exactCoords = match.history?.exactCoords || {};
            const scorePct = Math.max(1, Math.min(99, Math.round((match.score / (result.bestScore || match.score || 1)) * 100)));
            return `
              <div class="shrink-predictor-match">
                <div class="shrink-predictor-match-top">
                  <div class="shrink-predictor-match-title">${index + 1}. ${escapeHtml(coinDbCanonicalLabel(match.history?.entry?.coin_label || 'Historical match') || 'Historical match')}</div>
          ...
          : '<div class="shrink-predictor-empty">No historical exact-ended circles matched this shape yet.</div>';
          ...
          <h4>Prediction Coin Location</h4>
            <div class="shrink-predictor-grid">
              ...
              <div class="shrink-predictor-stat">
                ...
                <span class="shrink-predictor-stat-value">${escapeHtml(confidenceLabel)}</span>
              </div>
              ${Number.isFinite(backtestMean) ? `
                <div class="shrink-predictor-stat">
                  <span class="shrink-predictor-stat-label">Backtest Mean Error</span>
        ...
        if (typeof mapgl !== 'undefined' && mapgl) {
          ...
          for (const sourceId of [
            ...
            if (mapgl.getSource?.(sourceId)) {
              try { mapgl.removeSource(sourceId); } catch { }
            }
          }
        }

        if (mapleaf && shrinkPredictorLeafletLayer) {
          try { mapleaf.removeLayer(shrinkPredictorLeafletLayer); } catch { }
          shrinkPredictorLeafletLayer = null;
        }
      }

      function renderShrinkPredictorOverlay(result) {
        clearShrinkPredictorOverlay();
        if (!result?.predictedPoint || !result?.observedSteps?.length) return;
        ...
        if (engine === 'gl' && mapgl?.getStyle?.()) {
          if (!mapgl.getSource(SHRINK_PREDICTOR_SRC)) {
            mapgl.addSource(SHRINK_PREDICTOR_SRC, {
              type: 'geojson',
              data: {
                type: 'FeatureCollection',
          ...
          if (!mapgl.getSource(SHRINK_PREDICTOR_MATCH_SRC)) {
            mapgl.addSource(SHRINK_PREDICTOR_MATCH_SRC, { type: 'geojson', data: { type: 'FeatureCollection', features: matchFeatures } });
          } else {
            mapgl.getSource(SHRINK_PREDICTOR_MATCH_SRC).setData({ type: 'FeatureCollection', features: matchFeatures });
          }

          if (!mapgl.getSource(SHRINK_PREDICTOR_LINE_SRC)) {
            mapgl.addSource(SHRINK_PREDICTOR_LINE_SRC, { type: 'geojson', data: lineFeature });
          } else {
            mapgl.getSource(SHRINK_PREDICTOR_LINE_SRC).setData(lineFeature);
          }

          if (!mapgl.getSource(SHRINK_PREDICTOR_CONF_SRC)) {
            mapgl.addSource(SHRINK_PREDICTOR_CONF_SRC, { type: 'geojson', data: confidenceFeature });
          } else {
          ...
          if (!mapgl.getLayer('shrink-predictor-confidence-line')) {
            mapgl.addLayer({
          ...
          if (!mapgl.getLayer('shrink-predictor-line')) {
            mapgl.addLayer({
          ...
          if (!mapgl.getLayer('shrink-predictor-match-points')) {
            mapgl.addLayer({
              id: 'shrink-predictor-match-points',
              type: 'circle',
              source: SHRINK_PREDICTOR_MATCH_SRC,
              paint: {
                'circle-radius': 5,
                'circle-color': '#fef3c7',
                'circle-stroke-color': '#f59e0b',
          ...
          if (!mapgl.getLayer('shrink-predictor-point')) {
            mapgl.addLayer({
              id: 'shrink-predictor-point',
              type: 'circle',
              source: SHRINK_PREDICTOR_SRC,
              paint: {
                'circle-radius': 7,
                'circle-color': '#22c55e',
          ...
          for (const match of (result.matches || []).slice(0, 4)) {
            layers.push(L.circleMarker([match.predictedPoint.lat, match.predictedPoint.lng], {
              radius: 5,
              color: '#f59e0b',
              weight: 1.5,
              fillColor: '#fef3c7',
      ...
      async function runShrinkPredictor(modeOverride = '') {
        ...
        try {
          const observation = shrinkPredictorFindCurrentObservation(
            currentShrinkPredictorContext.fid,
            currentShrinkPredictorContext.center,
      ...
      function openShrinkPredictorModal(fid, center, radiusMeters, feature, featureProps = {}) {
        ...
        currentShrinkPredictorContext = {
          ...
          featureProps: Utils.deepClone(featureProps || {}),
          featureName: String(featureProps?.name || feature?.properties?.name || 'Selected circle')
        };
      ...
      shrinkPredictorModal?.addEventListener('click', (e) => {
        if (e.target === shrinkPredictorModal) closeShrinkPredictorModal();
      });
