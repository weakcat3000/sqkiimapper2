...
def load_excel_records(workbook_path: Path, min_prefix_steps: int) -> List[Record]:
    ...
    for row in ws.iter_rows(values_only=True):
        ...
        if row_counter % 500 == 0:
            print(f"  scanned {row_counter} workbook rows...")
        values = list(row)
    ...
    records: List[Record] = []
    print(f"Workbook grouped into {len(grouped_rows)} coin blocks. Building exact-ended records...")
    for coin_id, rows in grouped_rows.items():
        ...
        if len(records) % 25 == 0:
            print(f"  built {len(records)} exact-ended records so far...")
    print(f"Workbook load complete: {len(records)} exact-ended records ready.")
    return records
...
def build_samples(records: Sequence[Record], min_prefix_steps: int, max_prefix_samples_per_coin: int = 0) -> Tuple[List[Sample], FeatureStats]:
    samples: List[Sample] = []
    print(f"Building prefix samples from {len(records)} records (max per coin = {max_prefix_samples_per_coin or 'ALL'})...")
    for record_index, record in enumerate(records, start=1):
        ...
        if record_index % 10 == 0 or record_index == len(records):
            print(f"  processed {record_index}/{len(records)} records -> {len(samples)} prefix samples")
    ...
    print(f"Sample build complete: {len(samples)} total prefix samples.")
...
def predict_neighbors(model: Dict[str, Any], query_steps: Sequence[Step], query_profile: Dict[str, float], query_signature: Sequence[float], query_embedding: Sequence[float], config: Dict[str, Any], indices: Optional[Sequence[int]], exclude_entry_id: str = "", room_code: str = "", limit: int = K_NEIGHBORS) -> Dict[str, Any]:
    ...
    radius_ratio = float(query_profile.get("radiusRatio", 0.0))
    shrink_fraction = float(query_profile.get("shrinkFraction", 0.0))
    for sample in model["samples"]:
        ...
        radius_penalty = abs(radius_ratio - float(sample.profile.get("radiusRatio", 0.0))) * float(penalties["radius"])
        ...
        phase_penalty = abs(shrink_fraction - float(sample.profile.get("shrinkFraction", 0.0))) * float(penalties["radius"]) * 0.55
        size_norm_penalty = abs(radius_ratio - float(sample.profile.get("currentRadiusKm", 0.0)) / max(0.001, float(sample.profile.get("startRadiusKm", 1.0)))) * 0.35
        bundle_penalty = (
        ...
        total = embed_distance + radius_penalty + drift_penalty + cadence_penalty + size_penalty + duration_penalty + shrink_penalty + phase_penalty + size_norm_penalty + bundle_penalty + signature_penalty
